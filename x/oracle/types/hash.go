...
// SHA256("{salt},{feeder},{src_id}:{ts}:{symbol}:{usd_price},...,{src_id}:{ts}:{symbol}:{usd_price}")
type AggregateVoteHash []byte

// GetAggregateVoteHash computes hash value of PricesVote to avoid
// redundant DecCoins stringify operation.
func GetAggregateVoteHash(saltHex, PricessStr string, feeder sdk.AccAddress) AggregateVoteHash {
	...
	sourceStr := bytes.Join([][]byte{salt, feeder, []byte(PricessStr)}, []byte(","))
	fmt.Print("sourceStr: ", sourceStr, "\n")
	return tmhash.SumTruncated(sourceStr)
}
